...
Витрины Rutube - способ организации и представления видео-контента, используемый на портале [rutube.ru](https://rutube.ru). Одним из примеров контента, представленного с помощью витрин, может служить главная страница портала. Используя API партнеры Rutube имеют возможность разработать подобный тип представления видео-роликов на своей площадке.
...
_Совет! Рекомендуется рассматривать запросы к источникам одной вкладки как deferred-объекты. Это обеспечит равномерность работы интерфейса при загрузке и отрисовке карточек._
...
`origin__type` - фильтр роликов конкретной платформы. Возможные значения:

`rtb` - ролики Rutube, исключая live-трансляции;

`rst` - live-трансляции Rutube;
